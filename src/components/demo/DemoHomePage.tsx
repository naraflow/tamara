...
export const DemoHomePage: React.FC<DemoHomePageProps> = ({
  ...
  return <div className="space-y-16 p-8">
      ...
      <section className="text-center py-16">
        <h1 className="text-5xl font-bold mb-6">Tamara
...
Penanganan keluhan instan melalui WhatsApp. Sederhana. Cepat. Efektif.</h1>
      ...
      <section className="grid md:grid-cols-3 gap-6">
        {kpiCards.map((kpi, index) => <Card key={index} className="p-8 bg-gradient-to-br from-tamara-50 to-white border-0 shadow-lg transition-all hover:shadow-xl hover:-translate-y-1">
      ...
      <section>
        ...
        <div className="space-y-8">
          {workflowSteps.map(step => <div key={step.number} className="flex items-start space-x-6">
              {/* Step Number */}
              ...
              <div className="flex-1 pt-1">
                ...
                {step.description ? <p className="text-muted-foreground leading-relaxed">
                    {step.description}
                  </p> : <ul className="space-y-2">
                    {step.items?.map((item, index) => <li key={index} className="flex items-start space-x-3">
